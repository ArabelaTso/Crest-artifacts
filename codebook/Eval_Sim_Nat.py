...
This script is adapted from
    AEON: A Method for Automatic Evaluation of NLP Test Case (ISSTA2022)
"""
...
def edit_distance(ori_tokens, adv_tokens):
    ...
    for i in range(1, ori_len + 1):
        operation[i][0] = operation[i - 1][0] + ',d %d %d;a %d %d' % (
            i - 1, ori_tokens[i - 1], i - 1, ori_tokens[i - 1])
    for j in range(1, adv_len + 1):
        operation[0][j] = operation[0][j - 1] + ',a %d %d;d %d %d' % (
            j - 1, adv_tokens[j - 1], j - 1, adv_tokens[j - 1])
    ...
    for i in range(1, ori_len + 1):
        for j in range(1, adv_len + 1):
            ...
            if matrix[i][j] == matrix[i - 1][j - 1] + cost:
                ...
                operation[i][j] = operation[i][j - 1] + ',a %d %d;d %d %d' % (
                    i, adv_tokens[j - 1], j - 1, adv_tokens[j - 1])
            elif matrix[i][j] == matrix[i - 1][j] + 1:
                operation[i][j] = operation[i - 1][j] + ',d %d %d;a %d %d' % (
                    i - 1, ori_tokens[i - 1], j, ori_tokens[i - 1])

    return matrix[-1][-1], operation[-1][-1]


class AeonScorer(object):
    ...
    def pair_texts_similarity(self, ori_sentences, adv_sentences):
        ...
        with torch.no_grad():
            ...
            for i in range(len(ori_inputs)):
                ...
                ori_sentence_emb.append(
                    output.pooler_output if self.pooler == 'cls' else output.last_hidden_state[:, 0])
    ...
    def compute(self, ori_texts, adv_texts, save_file=''):
        ...
        for i, (ori, adv) in enumerate(zip(ori_texts, adv_texts)):
            ...
            nat_diff = abs(o_naturalness - a_naturalness)
            # ret.append([similarity, o_naturalness, a_naturalness, nat_diff])
            ret['semSim'].append(similarity)
            ret['o_Nat'].append(o_naturalness)
            ret['g_Nat'].append(a_naturalness)
            ret['natDiff'].append(nat_diff)
...
def parse_config():
    ...
    parser.add_argument('--embed-lm', type=str, default='princeton-nlp/sup-simcse-bert-base-uncased',
                        help='PLM for emebdding.')
    parser.add_argument('--batch-size', type=int, default=8, help='Batch size.')
    parser.add_argument('--ori-Output', type=str, default='Output/test_ori.txt', help='Original texts, one per line.')
    parser.add_argument('--adv-Output', type=str, default='Output/test_adv.txt', help='Adversarial texts, one per line.')
...
def extractManualResult_Both(file):
    # extract manual label results
    ...
    for line in lines:
        if line.startswith('CorefChanged'):
            corefChanged.append(line.strip().split(':')[-1].strip(' ') == 'True')
        if line.startswith('buggy'):
            isBuggy.append(line.strip().split(':')[-1].strip(' ') == 'True')

    # print(corefChanged)
    # print(isBuggy)
    return isBuggy, corefChanged


def extractManualResult_Buggy(file):
    # extract manual label results
    lines = open(file, 'r').readlines()
...
if __name__ == '__main__':
    ...
    scorer = AeonScorer(args.batch_size, args.masked_lm, args.embed_lm, args.verbose)

    genMethodNameList = ['toy']
    genMethodNameList = ['Corin', 'SIT', 'PatInv', 'CAT']  # 'WordReplace_CorefAware'

    for genMethodName in genMethodNameList:
        # tsv_path = '../OutputVal/corin_output_{}.tsv'.format(genMethodName)
        tsv_path = '../Output/Sample/label/Sampled100_{}.tsv'.format(genMethodName)
        # output_tsv_path = '../Output/Sample/label/Eval_Sem_Nat_{}.tsv'.format(genMethodName)
        output_tsv_path = '../Output/Sample/label/label_{}.tsv'.format(genMethodName)

        df = pd.read_csv(tsv_path, delimiter='\t')

        # ori_texts, adv_texts = df['oriSent'].tolist(), df['genSent'].tolist()
        #
        # result = scorer.compute(ori_texts, adv_texts, args.save_file)
        #
        # for key, val in result.items():
        #     df[key] = val

        manual_txt = '../Output/Sample/label/label100_{}.txt'.format(genMethodName)
        isBuggy, corefChanged = extractManualResult_Both(manual_txt)

        df['CorefChanged'] = corefChanged
        df['isBuggy'] = isBuggy
